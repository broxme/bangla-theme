...
$links = (get_theme_mod( 'bangla_toolbar_social' )) ? explode(',', get_theme_mod( 'bangla_toolbar_social' )) : null;
if (count($links)) : ?>
	<div class="social-link">
		<ul<?php echo bangla_helper::attrs($attrs_grid) ?>>
		    <?php foreach ($links as $link) : ?>
		    <li>
		        <a<?php echo bangla_helper::attrs(['href' => $link], $attrs); ?> broxme_wp-icon="icon: <?php echo bangla_helper::icon($link); ?>; ratio: 0.8" title="<?php echo ucfirst(bangla_helper::icon($link)); ?>" broxme_wp-tooltip="pos: bottom"></a>
		    </li>
		    <?php endforeach ?>
		</ul>
	</div>
